...
/*                                                                      */
/************************************************************************/
int *** duplicate_empty_index (int ***r)
{
  int a,b,c;
  int d1,d2,d3;
...
int *** declare_empty_index (Sequence *S)
{
  int ***r;
  int a,b,c;
...
Constraint_list * declare_constraint_list ( Sequence *S, char *name, int *L, int ne,FILE *fp, int **M)
    ...
    //New data structure
    CL->empty_index=0;
	CL->cache.resize( S->nseq , std::vector<std::pair<int,Constraint *> >( S->max_len+1 ) );
	CL->scores.resize(S->nseq , std::vector<tsl::sparse_map<size_t, int>>( S->nseq ) );
	CL->cache_it.resize((CL->S)->max_len+1,0);
	//CL->qChunk = 10;
	//CL->cass_ip = "192.168.1.1";
    return CL;
    }
...
Constraint_list *copy_constraint_list (Constraint_list *CL, int mode)
    ...
    /*DATA*/
      ...
      if ( mode==HARD_COPY)
	{
	  NCL->empty_index=CL->empty_index;
	}
      else NCL->empty_index=CL->empty_index;
...
Sequence *free_constraint_list (Constraint_list *CL)
    ...
    */
    ...
    if ( CL->empty_index)CL->empty_index=0;
