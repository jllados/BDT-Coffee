...
Constraint_list *  freeze_constraint_list (Constraint_list *CL)
{
	...
	for ( a=0; a<S->nseq; a++)
	  {
	    b=1;
	    while (CL->empty_index)
	      {
		  d+=CL->empty_index;
		  freeze[a][b]=CL->empty_index;
		  b++;
	      }
	  }
	CL->freeze=freeze;
...
Constraint_list *  empty_constraint_list (Constraint_list *CL)
{
  ...
  if ( !CL || ! CL->empty_index) return CL;
...
// 	static int *entry;
	...
	int b,c,s1, r1, s2, r2;
	int d=0;
	if (!CL || !CL->S || !CL->empty_index || CL->ne==0) return 0;
	S=CL->S;
		...
		for (r1=1; r1<=S->len[s1]; r1++)
		{
			...
			for (;b<CL->empty_index; b+=ICHUNK)
			{
				...
				else
				{
					...
					{
						d++;
						entry[WE]=CL->empty_index;
						entry[CONS]=CL->empty_index;
						entry[MISC]=CL->empty_index;
	...
	for (s1=0; s1<S->nseq; s1++)
	  {
	    ...
	    while (CL->empty_index)
	      {
		for (b=1; b<CL->empty_index; b+=ICHUNK)
		  {
		    s2=CL->empty_index;
		    r2=CL->empty_index;
		    w2=CL->empty_index;
		...
		for ( r1=1; r1<=S->len[s1]; r1++)
		  {
		    ...
		    for (a=1; a<CL->empty_index; a+=ICHUNK)
		      {
			int s2=CL->empty_index;
			int r2=CL->empty_index;
			int w2=CL->empty_index;
	...
	for (; s<(CL->S)->nseq; s++)
	  {
	    while ((CL->empty_index))
	      {
		for (; l<CL->empty_index;)
		  {

		    entry[SEQ1]=s;
		    entry[R1]=r;
		    entry[SEQ2]=CL->empty_index;
		    entry[R2]=  CL->empty_index;
		    entry[WE]=  CL->empty_index;
		    entry[CONS]=CL->empty_index;
		    entry[MISC]=CL->empty_index;
		    entry[INDEX]=l;
			...
			for (b=1; b<CL->empty_index; b+=ICHUNK)
			{
				...
				if (b>1)
				{
					...
					else if ( s2<=ps2 && r2<pr2)
					{
						HERE ("%s -- NOT SORTED",t);
						fprintf ( stderr, "\n");
						for (c=1; c<CL->empty_index; c++)
						{
							fprintf (stderr, "%5d ",CL->empty_index);
						}
...
int checkCL (Constraint_list *CL, char *t)
{
	...
	for (a=0; a<(CL->S)->nseq; a++)
		for (b=1; b<=(CL->S)->len[a]; b++)
		{
			HERE ("%s %4d ==> %d",t,CL->empty_index, CL->empty_index%ICHUNK);
		}


		if (!CL){HERE ("CL is Not declared");fail=1;}
		else if ( !CL->S){HERE ("S is not declared in CL");fail=1;}
		else if ( !CL->empty_index){HERE ("residue index is not declared");fail=1;}
		else if (read_array_size_new (NULL)!=(CL->S)->nseq){HERE ("CL not well declared (S)");fail=1;}
		else
		{
			for (a=0; a<(CL->S)->nseq; a++)
			{
				int s=(CL->S)->len[a]+1;
				int j=read_array_size_new (NULL);
				if (s!=j)fail=1;
...
Constraint_list *update_entry (CLIST_TYPE *entry, Constraint_list *CL, int i)
{
  ...
  CL->empty_index=entry[SEQ2];
  CL->empty_index  =entry[R2];
  CL->empty_index  =MAX(entry[WE],CL->empty_index);
  CL->empty_index+=entry[CONS];
  CL->empty_index=entry[MISC];
  return CL;
}
...
CLIST_TYPE *main_search_in_list_constraint ( int *key,int *p,int k_len,Constraint_list *CL)
{
	...
	{
		if (CL->empty_index==s2 && CL->empty_index==r2)
		{
			l[SEQ1]=s1;
			l[R1]=r1;
			l[SEQ2]=s2;
...
Constraint_list *sort_constraint_list(Constraint_list *CL, int start, int len)
{
	...
	for (s1=0; s1<S->nseq; s1++)
	{
		for (r1=1; r1<=S->len[s1]; r1++)
		{
			int nE=(CL->empty_index-1)/ICHUNK;
...
FILE * save_extended_constraint_list      (  Constraint_list *CL, char *mode, FILE *fp)
{
	...
	if (!CL || !CL->S || CL->empty_index)return fp;
	if ( !fp)fp=stdout;
...
Constraint_list * extend_constraint_list ( Constraint_list *CL)
{
	...
	static int *entry;
	if (!CL || !CL->empty_index || !CL->S)return CL;
	S=CL->S;
...
Constraint_list * relax_constraint_list (Constraint_list *CL)
{
  ...
  if (!CL || !CL->S || !CL->empty_index) return CL;
 ...
 *
 * Here we evaluate the consistency of edges in the Constraint_list::empty_index.
  ...
  for (sjobs=0,j=0;j<njobs; j++)
      ...
      if (vvfork (NULL)==0)
	{
	  ...
	  for (s1=sl[j][0]; s1<sl[j][1]; s1++)
		...
		{
		  norm1=0;
		  for (x=1; x< CL->empty_index; x+=ICHUNK)
		    {
		      t_s1=CL->empty_index;
		      t_r1=CL->empty_index;
		      hasch[t_s1][t_r1]=CL->empty_index;
		      norm1++;
		  ...
		  for ( a=1; a<CL->empty_index; a+=ICHUNK)
		      ...
		      for (x=1; x< CL->empty_index; x+=ICHUNK)
			{
			  t_s2=CL->empty_index;
			  t_r2=CL->empty_index;
			  if (t_s2==s1 && t_r2==r1)score+=(float)CL->empty_index;
			  else if (hasch[t_s2][t_r2])
			    {
			      score+=MIN((((float)hasch[t_s2][t_r2])),(((float)CL->empty_index)));
			    }
			  norm2++;
			}
...
//		{
//		  norm1=0;
//		  for (x=1; x< CL->empty_index[s1][r1][0]; x+=ICHUNK)
//		    {
//		      t_s1=CL->empty_index[s1][r1][x+SEQ2];
//		      t_r1=CL->empty_index[s1][r1][x+R2];
//		      hasch[t_s1][t_r1]=CL->empty_index[s1][r1][x+WE];
//		      /** Sascha: Sum up all weights / probabilities going out from here. */
//		      norm1 += hasch[t_s1][t_r1];
//
//		    }
//		  for ( a=1; a<CL->empty_index[s1][r1][0]; a+=ICHUNK)
//		    {
//		      score=0;
//		      norm2=0;
//		      s2=CL->empty_index[s1][r1][a+SEQ2];
//		      r2=CL->empty_index[s1][r1][a+R2];
//
//		      for (x=1; x< CL->empty_index[s2][r2][0]; x+=ICHUNK)
//			{
//			  t_s2=CL->empty_index[s2][r2][x+SEQ2];
//			  t_r2=CL->empty_index[s2][r2][x+R2];
//			  if (t_s2==s1 && t_r2==r1) score+= (float) CL->empty_index[s2][r2][x+WE];
//			  else if (hasch[t_s2][t_r2])
//			    {
//			      /** Sascha: Product of probabilities! See ProbCons paper */
//			      score+= ((float)hasch[t_s2][t_r2]) * ((float)CL->empty_index[s2][r2][x+WE]);
//			    }
//			  /** Sascha: Sum up all weights / probabilities going out from here. */
//			  norm2 += CL->empty_index[s2][r2][x+WE];
//			}
...
Constraint_list * expand_constraint_list_4gp (Constraint_list *CL, int T)
{
	...
	for (a=0; a<S->nseq; a++)//loop sequences
	{
		for (b=1; b<=S->len[a];b++)
		{
			for (c=1; c<CL->empty_index;c+=ICHUNK)
			{
				...
				for (d=c+3; d<CL->empty_index; d+=ICHUNK)
				{
					s2=CL->empty_index;
					r2=CL->empty_index;
					w=residue_pair_extended_list_4gp (CL,s1, r1,s2, r2);
...
int constraint_list2ne  ( Constraint_list *CL)
{
	...
	if (!CL || !CL->empty_index || !CL->S)return 0;
 ...
 *
 * \param[in,out] CL    The global Constraint_list object.
 * \param[in]     field Internal, constant number to access the CHUNKS of Constraint_list::empty_index correctly.
	  ...
	  for (;b<SL->empty_index; b+=ICHUNK)
	      ...
	      else
		{
		  ...
		  if (entry[R2]<=0)continue;
		    ...
		    {
		      entry[WE]=SL->empty_index;
		      entry[CONS]=SL->empty_index;
		      entry[MISC]=SL->empty_index;
		    }
...
Constraint_list *re_weight_constraint_list(Constraint_list * CL,Weights *W)
{
	...
	for (s=0; s<S->nseq; s++)
	{
		for ( r=1; r<=S->len[s]; r++)
		{
			for (a=1; a<CL->empty_index; a+=ICHUNK)
			{
				int t_s=CL->empty_index;
				CL->empty_index*=MIN(weight[s], weight[t_s]);
			}
		}
	}
	CL=evaluate_constraint_list_reference (CL);
			...
			if ( strm (mode, "slow"))
			  {
			    ...
			    if ( score>MAXID)score=(CL->empty_index)?sub_aln2sub_aln_score (B, CL, CL->evaluate_mode, ns, l_s):id;
	...
	for (s1=0; s1<S->nseq; s1++)
	{
		for (r1=1; r1<=S->len[s1]; r1++)
		{
			...
			for (c=1; c<CL->empty_index; c+=ICHUNK)
			{
				...
				n1=n2=0;
				list1[n1++]=r1;
				for (b=1; b<R->empty_index; b+=ICHUNK)
				{
					list1[n1++]=R->empty_index;
				}
				list2[n2++]=r2;

				for (b=1; b<R->empty_index; b+=ICHUNK)
				{
					list2[n2++]=R->empty_index;
				}
...
int cl2worst_seq (Constraint_list *CL, int *list, int ns)
{
  ...
  for (s1=0; s1<S->nseq; s1++)
	...
	{
	  s2=CL->empty_index;
	  score[s1][1]+=CL->empty_index;
	  score[s2][1]+=CL->empty_index;
	}
