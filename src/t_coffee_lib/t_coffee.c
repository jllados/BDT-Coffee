...
/*******************************************************************************************************/
	       ...
	       if ( get_type)
		 {
	        ...
	        */
			...
			/*for (a=0; a<S->nseq; a++) {
				S->len[a] = 10; //S->len[a]=strlen (S->seq[a]);
			 }*/
		...
		* Note that when running master, the library must be relaxed in order to compensate for missing pairs
	       ...
	       if ( (CL->S)->nseq>1 && CL->ne==0 && !CL->M &&!(do_convert && n_list>0))
		 {
		  ...
		  CL->seq_name2 = seq_name2;
		  CL->cass_ip = cass_ip;
		  CL->qChunk = qChunk;
		 //****
		 
		  FILE *fp;
		  char buff[50], cmd[200] = { 0 };
		  strcat(cmd, "cat /proc/meminfo | grep MemAvailable | awk '{ print ($2*1000)-500000000 }'");
	...
	//clock_t endt = clock();
	//fprintf(stdout, "%% TOTAL CLOCK TIME: %f %%\n", (double)(endt - begint) / CLOCKS_PER_SEC);
