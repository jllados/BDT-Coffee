...
int cl2pair_list_ext ( Alignment *A, int *ins, int **ils, Constraint_list *CL, int ***list_in, int *n_in)
{
  ...
  if (!CL || !CL->S || !CL->empty_index) return 0;
      ...
      if (vvfork (NULL)==0)
	{
	  ...
	  for (p1=sl[j][0]; p1<sl[j][1]; p1++)
	      ...
	      for (tot=0,nused=0,si=0;p1>0 && si<ns[0]; si++)
		{
		  ...
		  for (a=1; r>0 && a<CL->empty_index;a+=ICHUNK)
		    {
		      t_s=CL->empty_index;
		      t_r=CL->empty_index;
		      t_w=CL->empty_index;
...
int linked_pair_wise ( Alignment *A, int *ns, int **ls, Constraint_list *CL)
{
  ...
  if ( !CL->empty_index)return myers_miller_pair_wise (A, ns,ls,CL);
...
Constraint_list* collapse_list (Alignment *A,int *ns, int **ls, char **al, int len, Constraint_list *CL)
{
  ...
  for (cs1=0; cs1<(CL->S)->nseq; cs1++)
	...
	{
	  for (ll=l=1; l<CL->empty_index; l+=ICHUNK)
	    {
	      cs2=CL->empty_index;
		...
		{
		  p[SEQ1]=cs1;
		  p[SEQ2]=CL->empty_index;
		  p[R1]  =cr1;
		  p[R2]  =CL->empty_index;
		  p[CONS]=CL->empty_index;
		  p[MISC]=CL->empty_index;
		  p[WE]=CL->empty_index;
		  if (cs1==s1)
		    {
		      p[SEQ1]=s;
		...
		{
		  for (d=0; d<ICHUNK; d++) CL->empty_index=CL->empty_index;
		  CL->ne++;
		}
	    }
	  CL->empty_index=ll;
...
int cl2pair_list_collapse ( Alignment *A, int *ns, int **ls, Constraint_list *CL, int ***list_in, int *n_in)
{
  ...
  for (r1=1; r1<=l1; r1++)
    {
      tot=0; nused=0;
      for (a=1; r1>0 && a<CL->empty_index;a+=ICHUNK)
	{
	  t_s=CL->empty_index;
	  t_r=CL->empty_index;
	  t_w=CL->empty_index;
...
int procoffee_pair_wise ( Alignment *A, int *nsi, int **lsi, Constraint_list *CL)
{
  ...
  char **al;
  int len=0;
  if ( !CL->empty_index)return myers_miller_pair_wise (A, nsi,lsi,CL);
