...
static mru_list< std::pair<int,int> > lru_sc;
static float cache_elements = 0;
static float score_elements = 0;
...
int execute_query(const char* query, int s1, int cr1, Constraint_list *CL) {
	...
	cass_int32_t r2;
	cass_int32_t w;
	int iStart = cr1*CL->qChunk;
	int iEnd = iStart + CL->qChunk;
	int pid = getpid();
	static int aval_size = (CL->S)->nseq * ((CL->S)->max_len+1) * sizeof(int);
	
	if(!CL->session[pid]) {
		CL->session[pid] = cass_session_new();
		CL->cluster[pid] = create_cluster(CL->cass_ip);

		if (connect_session(CL->cluster[pid], CL->session[pid]) != CASS_OK) {
			cass_cluster_free(CL->cluster[pid]);
			cass_session_free(CL->session[pid]);
			return -1;
		}
	...
	future = cass_session_execute(CL->session[pid], statement);
	cass_future_wait(future);
	...
	if (rc == CASS_OK) {
		...
		if(size==0) {
			for(int i=iStart;i<iEnd && i<(CL->S)->max_len+1;i++) {CL->cache[s1][i].first = -1;}
		...
		for(int i=iStart;i<iEnd && i<(CL->S)->max_len+1;i++) {
			CL->cache[s1][i].second = (Constraint*) malloc(size * sizeof(Constraint));
		}
		...
		while (cass_iterator_next(iterator)) {
			...
			CL->cache[s1][r1].second[CL->cache_it[r1]].s2 = s2;
			CL->cache[s1][r1].second[CL->cache_it[r1]].r2 = r2;
			CL->cache[s1][r1].second[CL->cache_it[r1]].w = w;
			CL->cache_it[r1]++;
	...
	for(int i=iStart;i<iEnd && i<(CL->S)->max_len+1;i++) {
		CL->cache[s1][i].second = (Constraint*) realloc(CL->cache[s1][i].second, CL->cache_it[i] * sizeof(Constraint));
		CL->cache[s1][i].first = CL->cache_it[i];
		CL->cache_it[i]=0;
...
int sub_aln2sub_aln_score ( Alignment *A,Constraint_list *CL, const char *mode, int *ns, int **ls)
{
  ...
  score=(int)(CL->empty_index && CL->normalise)?((score*MAXID)/(CL->normalise)):(score);
  return (int)score;
}
...
Alignment * triplet_coffee_evaluate_output ( Alignment *IN,Constraint_list *CL)  
{
  ...
  if (!IN || !CL || !CL->empty_index) return IN;
	      ...
	      for (a=sl[j][0]; a<sl[j][1]; a++)
		{
		  ...
		  for (b=0; b<IN->nseq; b++)
		      ...
		      for (x=1;x<CL->empty_index;x+=ICHUNK)
			{
			  ...
			  r2=CL->empty_index;
			  w2=CL->empty_index;
			  for (y=1; y<CL->empty_index;y+=ICHUNK)
			    {
			      s3=CL->empty_index;
			      r3=CL->empty_index;
			      w3=CL->empty_index;
  ...
  for (a=0,s1=0; s1<A->nseq; s1++)
      ...
      else
	{
	  ...
	  for (r1=1;r1<=S->len[ls1]; r1++)
	    {
	      if (CL->empty_index>1){rseq[s1]=ls1;r1=S->len[ls1]+1;}
	    }
	  if (rseq[s1]!=-1)a++;
	}      
    }
...
int residue_pair_extended_list4rna ( Constraint_list *CL,Constraint_list *R, int s1, int r1, int s2, int r2 )
{
  ...
  list1[n1++]=r1;
  for (a=1; a<R->empty_index; a+=ICHUNK)
    {
	...
	for (a=1; a< CL->empty_index; a+=ICHUNK) 
	  {
		...
		if (hasch[t_s][t_r]==FORBIDEN)
		  {
		    score+=CL->empty_index;
		  }
		else 
		  {
		    delta=MIN(hasch[t_s][t_r],CL->empty_index);
		    score+=delta;
		  }
	      }
	  }
	...
	for (a=1; a< CL->empty_index; a+=ICHUNK) 
	  {
		...
		if (hasch[t_s][t_r]==FORBIDEN)
		  {
		    score+=((float)CL->empty_index/NORM_F);
		  }
		else 
		  {
		    delta=MAX((((float)hasch[t_s][t_r]/NORM_F)),(((float)CL->empty_index/NORM_F)));
		    score+=delta;
		  }
	      }
	  }
	...
	for (a=1; a< CL->empty_index; a+=ICHUNK) 
	  {
		...
		if (hasch[t_s][t_r]==FORBIDEN)
		  ...
		  {
		    delta=MIN((((float)hasch[t_s][t_r]/NORM_F)),(((float)CL->empty_index/NORM_F)));
		    score+=delta;
		  }
	      }
	  }
	...
	for (a=1; a< CL->empty_index; a+=ICHUNK) 
	  {
	    ...
	    if (hasch[t_s][t_r])
	      {
		if (field==WE){score+=2*MIN(hasch[t_s][t_r],CL->empty_index);}
	      }
	  }
...
int residue_pair_extended_list_g_coffee_quadruplet ( Constraint_list *CL, int s1, int r1, int s2, int r2 )
{
   int t_s, t_r, t_w, q_s, q_r, q_w;
	      ...
	      if ( CL->seq_for_quadruplet[t_s])
		{
		  for ( b=1; b<CL->empty_index; b+=ICHUNK)
		      ...
		      if (CL-> seq_for_quadruplet[q_s])
			{
			  ...
			  hasch[q_s][q_r]=MIN(CL->empty_index,t_w);
			}
		    }
		}
	    }
	  ...
	  for (s=0,score=0,a=1; a< CL->empty_index; a+=ICHUNK) 
	    {
	      t_s=CL->empty_index;
	      t_r=CL->empty_index;
	      t_w=CL->empty_index;
		  ...
		  for ( b=1; b<CL->empty_index; b+=ICHUNK)
		    {
		      ...
		      q_w=CL->empty_index;	
		      if (hasch[q_s][q_r] && CL->seq_for_quadruplet[q_s])
			s=MIN(hasch[q_s][q_r],MIN(CL->empty_index,q_w));
		      score=MAX(score, s);
		    }
		}
	    }
	...
	for (s=0, score=0,a=1; a< CL->empty_index; a+=ICHUNK) 
	  {
	    ...
	    if (hasch[t_s][t_r])
	      {
		if (field==WE)
		  {s=MIN(hasch[t_s][t_r],CL->empty_index);
		   score=MAX(s,score);
		  }
	      }
	  }
	...
	*/
	...
	if ( r1<=0 || r2<=0)return 0;
	   ...
	   for (a=1; a< CL->empty_index; a+=ICHUNK) 
	       ...
	       if (hasch[t_s][t_r])
		   {
		   if (field==WE)score+=MIN(hasch[t_s][t_r],CL->empty_index );
...
int get_dp_cost ( Alignment *A, int**pos1, int ns1, int*list1, int col1, int**pos2, int ns2, int*list2, int col2, Constraint_list *CL)
	...
	if (MODE!=2 || MODE==0  || (!CL->empty_index && CL->M) || (!CL->empty_index && CL->T)|| ns1==1 || ns2==1)
	  score=slow_get_dp_cost ( A, pos1, ns1, list1,col1, pos2, ns2, list2, col2, CL);
	...
	for ( a=n_pair-1; a>=0; a--)
	  {
	    ...
	    if ((s=(CL->evaluate_residue_pair)(CL, rs1, r1, rs2, r2))!=UNDEFINED) {
			score+=s;				    
		} else 
	      {
